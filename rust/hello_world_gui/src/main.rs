...
struct MyMacPlatform(Mutex<MyMacPlatformState>);
struct MyMacPlatformState {
    headless: bool,
}

struct MyMacDispatcher { parker: Arc<Mutex<Parker>>}



fn main() {
    let state = MyMacPlatformState { headless: false};
    let m = Mutex::new(state);
    let platform = MyMacPlatform(m);
    ...
    unsafe {
        ...
        let quit_prefix = NSString::alloc(nil).init_str("QQQuit ");
        let quit_title =
